...
const DebtCaseFormPage: FC<IPage> = (props): ReactElement => {
    ...
    const [editedDebtCaseData, setEditedDebtCaseData] = useState<IDebtCase | null>(null);
    ...
    const {
        data: debtCaseData,
        error: debtCaseError,
    } = useGet<IDebtCase>('debtcases/{id}', { id: Number(debtcaseId) });

    const {
        data: debtCaseTypeData,
    ...
    useEffect(() => handleAPIError(editDebtCaseError, openSnackbar, editCompleted), [editCompleted, editDebtCaseError, openSnackbar]);
    useEffect(() => handleAPIError(editDebtorError, openSnackbar, editCompleted), [editCompleted, editDebtorError, openSnackbar]);

    useEffect(() => setEditedDebtCaseData(debtCaseData ? { ...debtCaseData, debtCaseType: debtCaseData.debtCaseType || {} } : null), [debtCaseData]);
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, field: keyof IDebtCase) => {
        setFieldErrors((prevErrors) => ({ ...prevErrors, [field]: '' }));
        setEditedDebtCaseData((prevData) => {
            if (prevData) {
                return {
                    ...prevData,
                    [field]: field === 'debtCaseType' ? { ...prevData.debtCaseType, id: e.target.value } : e.target.value,
                };
            }
            return prevData;
        });
    ...
    const handleEditDebtorChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, field: keyof IDebtor) => {
        setFieldErrors((prevErrors) => ({ ...prevErrors, [field]: '' }));
                            ...
                            <Grid item xs={12} key={field}>
                                <div>
                                    ...
                                    {field === 'debtCaseType' ? (
                                        <TextField
                                            select
                                            value={(editedDebtCaseData && editedDebtCaseData.debtCaseType) ? (editedDebtCaseData.debtCaseType.id || '') : ''}
                                            onChange={(e) => handleInputChange(e, field as keyof IDebtCase)}
                                            style={{ width: '30%' }}>
                                        ...
                                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                                            <DateTimePicker
                                                ...
                                                onChange={(date) => handleDateTimeChange(date, field as keyof IDebtCase)}
                                            />
                                        </LocalizationProvider>
                                    ) : (
                                        <input
                                    ...
                                    <Grid item xs={12} key={field}>
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => handleEditDebtorChange(e, field as keyof IDebtor)}
                                                style={{ width: '30%' }}
                                            />
                                            <span style={{ color: 'red' }}>{fieldErrors[field]}</span>
                                        </div>
